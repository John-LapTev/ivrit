import { ALL_TOPICS, AUDIO_CACHE, DEFAULT_SESSION_LIMIT, DEFAULT_SPEECH_RATE, NIQQUD_HIDDEN_DEFAULT, SCRIPT_SHOWN_DEFAULT, STORE_EXAMS, STORE_GRAMMAR, STORE_HARD, STORE_LETTERS, STORE_SRS, STORE_STATS, STORE_WORDS, USER_LEVEL } from './core/constants.js';
...
import { el, fill, syncRangeFill, toast } from './ui/dom.js';
import { wordLine } from './ui/word.js';
import { allTopics, renderHome } from './ui/home.js';
import { uiIcon } from './ui/icons.js';
import { showScreen } from './ui/screens.js';
import { startRipple } from './ui/ripple.js';
import { exitTraining, handleKeydown, restartTraining } from './ui/train.js';
...
export async function loadEverything() {
  ...
  state.scriptShown = await getSetting('scriptShown', SCRIPT_SHOWN_DEFAULT);
...
function bindEvents() {
  ...
  // Тумблер темы: слушаем change, а не click — иначе клавиатурное переключение пробелом
  // сработает дважды, и тема вернётся обратно
  document.getElementById('settings-theme').addEventListener('change', (event) => {
    event.target.closest('.toggle').classList.add('is-live');
    toggleTheme();
  });
...
export async function start() {
  try {
    /* Сначала рисуем окно, потом грузим данные.
       Раньше было наоборот, и на телефоне человек несколько секунд смотрел в пустоту:
       открытие базы, засев 296 слов и загрузка указателя озвучки шли ДО первой отрисовки
       (жалоба Джона 04.09.2026 — «долгое время пустота»). Тяжёлое ушло в фон. */
    document.querySelectorAll('.tab-icon[data-icon]').forEach((slot) => {
      slot.append(uiIcon(slot.dataset.icon, 20));
    });
    /* Первый кадр бегунок обязан встретить стоя: без этого он проезжает всю панель
       слева направо, пока человек только открыл приложение. */
    const tabbar = document.getElementById('tabbar');
    tabbar.classList.add('no-anim');
    requestAnimationFrame(() => tabbar.classList.remove('no-anim'));
    showScreen('home');
    fill('now-card', el('div', { class: 'card center' }, [
      el('p', { class: 'faint', text: 'Готовлю словарь…' }),
    ]));
    ...
    /* Указатель озвучки грузится по сети и первую отрисовку задерживать не должен:
       без него приложение работает, просто у слов пока нет кнопки звука. */
    speech.init().then(() => { if (state.screen === 'home') renderHome(); });

    if (memoryOnly) {
      // Браузер не дал базу (приватный режим, кончилось место, file://).
      // Молчать нельзя: человек будет учиться, а прогресс не сохранится.
      toast('Прогресс не сохранится: браузер не дал доступ к хранилищу.', true);
    } else if (addedWords) {
    ...
    if ('serviceWorker' in navigator && location.protocol !== 'file:') {
      navigator.serviceWorker.register('sw.js').then((registration) => {
        // Проверка версии — сетевой запрос: офлайн он отказывает, и это нормально.
        // Без своего catch отказ всплывал глобальным «Что-то не сохранилось».
    ...
    document.querySelector('main').prepend(el('div', { class: 'card' }, [
      ...
      el('p', { class: 'faint', text: 'Чаще всего это значит, что файл открыт двойным кликом. Открой через локальный сервер: bash tools/serve.sh, адрес http://localhost:8322' }),
    ]));
  }
}
