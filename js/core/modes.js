import { ALL_TOPICS, EXAM_MIN_REPETITIONS, EXAM_PASS_SCORE, EXAM_QUESTION_COUNT, EXAM_READY_RATIO, FINAL_EXAM_LEVEL, FINAL_EXAM_QUESTIONS, FINAL_EXAM_RATIO, MAX_LEVEL, OPTIONS_PER_QUESTION, QUALITY_FORGOT, QUALITY_GOOD, QUALITY_HARD, STORE_EXAMS, STORE_SRS } from './constants.js';
...
export const MODES = [
  ...
  /* Диктант — мост от букв к словам: ухо слышит звук, рука пишет чтение. Механика
     (`byEar` у вопроса с вводом) была написана давно, но ни одна кнопка её не открывала. */
  { id: 'dictation', icon: 'ד', title: 'Диктант',
    description: 'Слово звучит, текста нет — печатаешь его чтение русскими буквами.', needsVoice: true },
  /* Чтение от руки — отдельный навык, а не украшение. Печатная буква к рукописной не ключ:
     א от руки похожа на «lc», ה на «n», ל на петлю. Человек, знающий только печать,
     не прочитает ни записку, ни ценник, ни заполненный бланк — и, увидев такой текст,
     теряется (жалоба владельца 09.09.2026). Здесь слово показано ТОЛЬКО рукописным. */
  { id: 'script', icon: 'י', title: 'От руки',
    description: 'Слово написано от руки, печатного нет — узнаёшь его и выбираешь перевод.',
    needsVoice: false },
];
...
export const session = {
  ...
  wrong: 0,
  answered: false,
  typed: '',          // что человек напечатал: поле после ответа показывает это же
  flipped: false,
  question: null,
...
function buildQuestion(word, pool, mode) {
  if (mode === 'heb2ru' || mode === 'listen' || mode === 'script') {
    ...
    return { kind: 'choice', mode, word, options: choice.options, correctIndex: choice.correctIndex,
      // handOnly — слово показано только рукописным: печатное подсказало бы ответ
      optionField: 'translation', hideWord: mode === 'listen', handOnly: mode === 'script' };
  }
  if (mode === 'ru2heb') {
...
export function startSession(options) {
  ...
  /* Экзамену даём СВОЙ счёт на каждый заход: `session.restart` держит те же настройки,
     и без копии «Заново» продолжало бы считать ответы прошлой попытки — двадцать вопросов
     подряд набирали проходной балл за две проваленные попытки. */
  session.exam = options.exam ? Object.assign({}, options.exam, { asked: 0, correct: 0 }) : null;
...
export function nextQuestion() {
  ...
  /* Неверные варианты берём только из открытых слов: иначе среди ответов всплывают слова
     закрытых уровней, которых человек в глаза не видел (отзыв 03.09.2026 — «что это за
 ...
 * @returns {Promise<boolean>} удалось ли записать ответ в базу; сказать об отказе —
 *   дело экрана (`ui/train.js`), потому что `core` про сообщения ничего не знает
 */
export async function recordAnswer(word, verdict, explicitQuality) {
  ...
  if (session.exam) {
    ...
    /* Экзамен не двигает интервалы, но день, проведённый за ним, — не прогул: без этой
       записи серия занятий рвалась ровно так же, как рвалась на трудных словах. */
    try {
      const askedMode = (session.question && session.question.mode) || session.mode;
      await updateDayStats({ reviewed: 1, correct: verdict === 'correct' ? 1 : 0,
        errors: verdict === 'correct' ? 0 : 1, mode: `exam:${askedMode}` });
    } catch (error) {
      saved = false;
    }
    return saved;
  ...
  try {
    ...
    // не работали и ни слова на экране (аудит 03.09.2026). Теперь ответ доигрывается
    // до конца, а сказать человеку о неудаче — дело экрана: core о нём не знает.
    saved = false;
  }
...
/* Свои слова лежат на уровне 0 и потому доступны всегда, слова уровней — по мере сдачи
   экзаменов. Плюс всё, что уже дала программа занятий: раз слово показали на занятии,
   прятать его в тренировке и в словаре бессмысленно — человек его учит. */
export const isWordAvailable = (word) => word.level <= state.unlockedLevel
  || isStarted(state.srs.get(word.id));
