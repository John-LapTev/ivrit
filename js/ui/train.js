import { MAX_LEVEL, QUALITY_EASY, QUALITY_FORGOT, QUALITY_HARD, STEP_PASS_SHARE } from '../core/constants.js';
...
import { bigWord, handWord, hebText, wordLine } from './word.js';
...
export function beginTraining() {
  ...
  if (mode.needsVoice && !speech.available) {
    toast('Для этого режима нужен звук: включи голос в системе.', true);
    return;
  }
  ...
  if (!words.length) {
    // Звук есть, а озвученных слов среди повторяемых нет — это другая беда, и сказать о ней
    // надо иначе, иначе человек пойдёт «проходить новые слова», а режим всё равно смолчит
    toast(mode.needsVoice
      ? 'Для этого режима нет озвученных слов: возьми слова из словаря приложения.'
      : 'Нечего повторять — сначала пройди новые слова.');
    return;
  }
...
export function renderTrain(feedback) {
  ...
  if (!session.question) {
    // Итог пишет результат экзамена в базу. Откажет — карточка всё равно должна появиться,
    // иначе человек остаётся на пустом экране без единой кнопки
    renderSessionSummary().catch((error) => {
      session.active = false;
      fill('train-body', el('div', { class: 'train-card' }, [
        el('div', { class: 'card-question', text: 'Занятие закончено' }),
        el('p', { class: 'faint', text: `Результат не сохранился: ${error.message}` }),
      ]));
      fill('train-actions', el('button', { class: 'btn', type: 'button', onclick: exitTraining }, 'На главную'));
    });
    return;
  }
  ...
  if (feedback) {
    // Сообщение бывает набором узлов: слово на иврите внутри русской фразы обязано ехать
    // отдельным островком, иначе тире на границе перепрыгивает внутрь него
    fill(verdictNode, Array.isArray(feedback.message) ? feedback.message : [feedback.message]);
...
function renderChoiceQuestion(question, feedback) {
  ...
  } else if (question.handOnly) {
    /* Режим «От руки»: слово написано рукописным, и до ответа рядом не должно быть
       ни печатного начертания, ни чтения — иначе это не проверка, а показ.
       После ответа появляется печатная пара: именно на ней и видно соответствие. */
    cardChildren.push(handWord(word.heb));
    if (showAnswer) {
      cardChildren.push(el('p', { class: 'faint', text: 'Печатными это выглядит так:' }));
      cardChildren.push(bigWord(word.heb, 64));
      cardChildren.push(el('div', { class: 'card-translit', text: word.translit }));
      if (speech.available) cardChildren.push(speakButton(word.heb));
    } else {
      cardChildren.push(el('p', { class: 'faint', text: 'Так это слово пишут от руки — что оно значит?' }));
    }
  ...
  /* Кружок помечает изучаемое ИВРИТСКОЕ слово. Поэтому он стоит у вариантов только там,
     ...
     слово бессмысленно (отзыв 03.09.2026: «я же не русское слово учу») — там кружок один,
  ...
  const options = question.options.map((option, index) => {
    ...
    const button = el('button', {
      ...
      markOptions ? wordLine(option.heb, 'option-body')
        : el('span', { class: 'option-body', text: option[question.optionField] }),
    ]);
...
/**
 * Засчитать ответ и, если база отказала в записи, сказать об этом человеку.
 * Сообщение живёт здесь, а не в `core/modes.js`: слои не перепрыгиваются.
 */
async function saveAnswer(word, verdict, quality) {
  const saved = await recordAnswer(word, verdict, quality);
  if (!saved) {
...
async function answerUnknown() {
  ...
  await saveAnswer(word, 'wrong');
  renderTrain({ verdict: 'wrong',
    message: [wordLine(word.heb), ` — ${word.translit} — ${word.translation}`] });
}
...
function renderInputQuestion(question, feedback) {
  ...
  /* После ответа поле не очищается: человек должен видеть, что именно он написал,
     рядом с правильным чтением. Ошибка вдобавок отзывается тряской — на телефоне
     красная рамка под пальцем не видна, а толчок чувствуется. */
  const verdictClass = !feedback ? null
    : feedback.verdict === 'correct' ? 'is-right' : 'is-wrong';
  const input = el('input', {
    type: 'text', id: 'translit-input', autocomplete: 'off', autocapitalize: 'off',
    spellcheck: 'false', disabled: session.answered,
    class: verdictClass, value: session.answered ? session.typed : '',
  });
...
export async function answerChoice(index) {
  ...
  await saveAnswer(question.word, correct ? 'correct' : 'wrong');
  const right = question.options[question.correctIndex];
  renderTrain({
    verdict: correct ? 'correct' : 'wrong',
    chosenIndex: index,
    message: correct ? 'Верно'
      : ['Правильно: ', wordLine(right.heb), ` — ${right.translit} — ${right.translation}`],
  });
...
async function answerTranslit(value) {
  ...
  await saveAnswer(question.word, verdict);
...
async function answerFlashcard(quality) {
  ...
  await saveAnswer(session.question.word, verdict, quality);
  advanceQuestion();
}
...
async function renderSessionSummary() {
  ...
  /* Шаг программы закрывается по доле верных: безошибочный проход с первого раза —
     недостижимая планка для нового материала, а ошибки и так вернутся повторением. */
  const answered = session.correct + session.wrong;
  const clean = answered > 0 && Math.round((session.correct / answered) * 100) >= STEP_PASS_SHARE;
  const actions = [];
  if (fromTeacher && !clean) {
    actions.push(el('p', { class: 'faint center', style: 'margin-bottom:12px',
      text: `Шаг закроется от ${STEP_PASS_SHARE} % верных. Ошибки уже в очереди — прогони их.` }));
