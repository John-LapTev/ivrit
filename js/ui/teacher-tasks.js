import { OPTIONS_PER_QUESTION, STEP_PASS_SHARE, STORE_SRS } from '../core/constants.js';
...
import { applyPractice, applySm2, createSrsRecord } from '../core/srs.js';
...
export function exitTeacherTask() {
  releaseMicrophone();
  speech.stop();
  if (state.teacherTask) state.teacherTask.playingGroups = false;
...
function shadowingBlock(task, heb) {
  ...
  const playAll = async () => {
    ...
    for (const group of groups) {
      // Ушли с экрана посреди проигрывания — замолкаем: иначе иврит звучит поверх главной
      if (!task.playingGroups || state.screen !== 'teacher-task') { task.playingGroups = false; return; }
...
function renderTaskSummary(task) {
  ...
  /* Шаг закрывается по доле верных, а не безошибочным проходом. Требовать 100 % с первого
     раза на каждом из семи-восьми шагов дня — значит наказывать за то, ради чего пришли:
     ошибка в новом материале нормальна, и слова из неё всё равно возвращаются повторением. */
  const clean = task.exam ? passed : share >= STEP_PASS_SHARE;
  ...
  fill('teacher-task-body', el('div', { class: 'card center' }, [
    ...
    el('p', { class: 'faint', text: task.exam
      ...
      : share >= STEP_PASS_SHARE ? 'Задание пройдено.'
 ...
 */
function countStudy(task, result, item) {
  updateDayStats({
    reviewed: 1,
  ...
  creditWords(result, item || task.items[Math.min(task.index, task.items.length - 1)])
    .catch(() => toast('Занятие не записалось в повторения: база не отвечает.', true));
}

/**
 * Занятие двигает повторения — иначе месяц по программе для приложения не существовал бы:
 * слова оставались «не в работе», уровни не открывались, «изученные слова» пустовали.
 *
 * Просмотр карточки — знакомство: слово заводится и считается показанным, но срок
 * следующего повторения не отодвигается. Ответ с проверкой идёт в SM-2 как обычный ответ
 * тренировки. У фразы засчитываются все её слова: она из них и собрана.
 * @param {'seen'|'correct'|'wrong'} result
 * @param {{heb: string}} item — слово или фраза задания
 */
async function creditWords(result, item) {
  if (!item || !item.heb) return;
  const byKey = wordsByKey();
  const touched = new Set();
  for (const key of splitPhrase(item.heb)) {
    const word = byKey.get(key);
    if (!word || touched.has(word.id)) continue;
    touched.add(word.id);
    const previous = state.srs.get(word.id) || createSrsRecord(word.id);
    const updated = result === 'seen'
      ? applyPractice(previous, true)
      : applySm2(previous, qualityFromVerdict(result === 'correct' ? 'correct' : 'wrong'));
    if (result === 'wrong') updated.errors += 1;
    state.srs.set(word.id, updated);
    await dbPut(STORE_SRS, updated);
  }
}
