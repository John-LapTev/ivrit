...
const drill = {
  ...
  wrong: 0,
  answered: false,
  typed: '',        // что набрали: поле после ответа показывает это же
  revealed: false,
  requeued: new Set(),
...
function startDrill(modeId, words) {
  ...
  /* Варианты обязательно сбрасывать: без этого новый заход открывался со списком ответов
     от прошлого вопроса — и правильного среди них не было вовсе (отзыв 03.09.2026). */
  drill.options = null;
  clearAttempt();
...
export function renderHardDrill(feedback) {
  ...
  if (feedback) {
    // Сообщение бывает набором узлов: слово на иврите едет отдельным островком,
    // иначе тире на границе с русским текстом перепрыгивает внутрь него
    ...
    /* Дальше — только по нажатию, даже когда ответ верный. Здесь слово не просто проверяют,
       ...
       (просьба отзыв 03.09.2026). В обычной тренировке всё осталось как было. */
...
async function answerChoiceStep(word, option, index) {
  ...
  await finishStep(word, correct, index, correct ? 'Верно'
    : ['Правильно: ', wordLine(word.heb), ` — ${word.translit}`]);
}
...
function renderTypeStep(word, feedback, byHeb) {
  // После ответа поле оставляет набранное и отзывается: верно — зелёная кромка,
  // мимо — красная и короткая тряска (на телефоне цвет под пальцем не виден, толчок — да)
  const verdictClass = !feedback ? null : feedback.correct ? 'is-right' : 'is-wrong';
  const input = el('input', {
    type: 'text', id: 'hard-input', autocomplete: 'off', autocapitalize: 'off',
    spellcheck: 'false', disabled: drill.answered,
    class: [byHeb ? 'heb' : null, verdictClass].filter(Boolean).join(' ') || null,
    value: drill.answered ? drill.typed : '',
...
async function answerTypeStep(word, value, byHeb) {
  ...
  await finishStep(word, correct, null, correct ? 'Верно'
    : byHeb ? ['Правильно: ', wordLine(word.heb), ` — ${word.translit}`]
...
function hardUnknownButton(word) {
  return el('button', {
    ...
    onclick: async () => {
      if (drill.answered) return;
      drill.answered = true;
      await finishStep(word, false, null,
        [wordLine(word.heb), ` — ${word.translit} — ${word.translation}`]);
    },
  }, 'Не помню — показать ответ');
}

/**
 * Счётчики слова растут и здесь: Просили, чтобы «изученные слова» считали все режимы.
...
async function countAnswer(word, correct) {
  ...
  // В память кладём сразу: заход не должен вставать из-за медленной или сломанной базы
  ...
  try {
    ...
    /* Раньше отказ базы обрывал обработчик посреди ответа и экран замирал — в тренировке
       это вылечили 03.09.2026, а тот же путь в трудных словах остался. Теперь заход
       доигрывается до конца, а человеку говорим прямо. */
    toast('Не удалось сохранить ответ: браузер отказал в записи. Заход лучше начать заново.', true);
  }
}

/** Отметка «режим пройден»: та же забота, что и о счётчиках — отказ базы не рвёт заход. */
async function passMode(word) {
  try {
    await markHardPassed(word.id, drill.mode);
  } catch (error) {
    toast('Отметку «режим пройден» не удалось сохранить — она пропадёт при перезагрузке.', true);
  }
}
