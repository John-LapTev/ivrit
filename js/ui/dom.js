 ...
 * @param {string|Array<string|Node>} message — строка или готовые узлы. Узлы нужны,
 *   когда в сообщении есть ивритское слово: оно обязано ехать отдельным островком
 *   (`wordLine`), иначе тире и точка на границе с русским текстом прыгают внутрь него.
 */
export function toast(message, isError) {
  const node = document.getElementById('toast');
  fill(node, Array.isArray(message) ? message : [message]);
...
/** Подпись в шапке. Обычно её ставит showScreen, но у экрана с вкладками она меняется
    и внутри экрана: раздел «Фразы» — это правила ИЛИ разговоры, а шапка про это молчала. */
export function setHeaderTitle(title) {
  document.getElementById('header-title').textContent = title;
}

/**
 * Доля заполнения ползунка в его собственную переменную --fill.
 * Chrome не умеет красить дорожку до бегунка сам (у Firefox для этого есть
 * ::-moz-range-progress), поэтому дорожка — градиент, а точку перелома ставим отсюда.
 */
