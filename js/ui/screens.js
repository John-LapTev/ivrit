...
import { setHeaderTitle } from './dom.js';
...
const TAB_FOR_SCREEN = { home: 'home', practice: 'home', train: 'home', alefbet: 'alefbet', dict: 'dict', add: 'dict', bulk: 'dict', writing: 'alefbet',
...
/**
 * Подводит бегунок под текущую вкладку. Ширины у вкладок равные (flex: 1 1 0), поэтому
 * достаточно передать в стили НОМЕР вкладки — остальное считает CSS. Замеры координат
 * тут были бы вредны: панель прячется в тренировке (display: none), и замер дал бы нули.
 */
function moveTabInk() {
  const bar = document.getElementById('tabbar');
  if (!bar) return;
  const tabs = [...bar.querySelectorAll('.tab')];
  const index = tabs.findIndex((tab) => tab.hasAttribute('aria-current'));
  if (index >= 0) bar.style.setProperty('--tab-i', String(index));
}
...
export function showScreen(name) {
  ...
  moveTabInk();
  setHeaderTitle(SCREEN_TITLES[name] || '');
