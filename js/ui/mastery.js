...
/* ——— Изученные слова: насколько слово закреплено ———
   Отзыв 03.09.2026: «хочу видеть не на глаз, а по цифрам, какие слова даются тяжелее».
...
/* Три верхних ранга носят свои имена (отзыв 03.09.2026: золотой, алмазный, рубиновый)
...
function wordTile(word, counts) {
  ...
  return el('button', {
    ...
    title: `${hebText(word.heb)} — ${word.translation}\nверно ${counts.right}, ошибок ${counts.wrong}, `
      + `счёт ${counts.score} · ${rank.title}`,
    'aria-label': `${hebText(word.heb)}: верно ${counts.right}, ошибок ${counts.wrong}`,
    ...
    // Кегль считается от полного слова, чтобы он не прыгал, когда огласовки спрятаны.
    wordLine(word.heb, 'mastery-heb', hebSize(word.heb)),
...
/** Образец ранга в легенде: та же рамка и то же свечение, но внутри одно название.
    Отзыв 03.09.2026: слово, чтение и цифры на образце лишние — легенда от них разъезжалась. */
