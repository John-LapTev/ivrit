...
export function renderHardScreen() {
  ...
  const children = [
    el('div', { class: 'card' }, [
      ...
      /* Человек прочитал «0 из 75» как число слов и не понял, откуда оно (03.09.2026):
  ...
  /* Кружок стоит у каждого варианта ответа, и вместе с нужным словом легко пометить
     обманку, которую и не учил (отзыв 03.09.2026: «слова „сын“ у нас вообще не было»).
  ...
  words.forEach((word) => {
    ...
    children.push(el('div', { class: `word-row${untouchedWord ? ' is-locked' : ''}` }, [
      ...
      speech.available ? el('button', {
        class: 'hard-round', type: 'button', 'aria-label': `Озвучить ${hebText(word.heb)}`,
        onclick: () => speech.speak(word.heb),
      }, uiIcon('sound', 16)) : null,
      el('button', {
        class: 'hard-round hard-drop', type: 'button', 'aria-label': `Снять метку с ${hebText(word.heb)}`,
...
export function hardCard(fromScreen) {
  ...
  const progress = hardProgress();
  return sideRow({
    title: `Трудные слова · ${words.length}`,
    note: progress.complete
      ? 'Круг пройден целиком — можно начать заново.'
      : `Помечено кружком, пройдено ${progress.percent}%. Отдельный прогон, на сроки повторений он не влияет.`,
