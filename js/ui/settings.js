...
import { askConfirm, el, fill, syncRangeFill, toast } from './dom.js';
...
const SCRIPT_CARD_ID = 'script-card';

/**
 * Тумблер: положение видно, не читая подписи. Кнопка «Показать/Скрыть» на его месте
 * называла ДЕЙСТВИЕ, а не состояние — и половину раз человек нажимал не туда.
 * Корпус и бегунок — отдельные узлы: перелив живёт слоем, а не псевдоэлементом.
 */
function toggleSwitch(options) {
  const input = el('input', {
    type: 'checkbox', id: options.id, 'aria-label': options.label,
    onchange: (event) => { event.target.closest('.toggle').classList.add('is-live'); options.onchange(); },
  });
  return el('label', { class: 'toggle' }, [
    input,
    el('span', { class: 'toggle-body', 'aria-hidden': 'true' }),
...
function ensureNiqqudCard() {
  ...
  const card = el('div', { class: 'card', id: NIQQUD_CARD_ID }, [
    el('div', { class: 'row-between' }, [
      ...
      toggleSwitch({ id: 'niqqud-toggle', label: 'Показывать огласовки', onchange: toggleNiqqud }),
    ]),
  ]);
  document.getElementById('settings-theme').closest('.card').before(card);

  const scriptCard = el('div', { class: 'card', id: SCRIPT_CARD_ID }, [
    el('div', { class: 'row-between' }, [
      el('div', {}, [
        el('b', { text: 'Рукописные буквы под печатными' }),
        el('div', { class: 'faint', text: 'В Израиле от руки пишут курсивом, и печатная буква '
          + 'к нему не ключ. Под каждой печатной стоит её рукописная пара — буква под буквой.' }),
      ]),
      toggleSwitch({ id: 'script-toggle', label: 'Рукописные буквы под печатными', onchange: toggleScriptPair }),
    ]),
  ]);
  card.after(scriptCard);
}

/** Рукописная пара под печатным словом — по всему приложению разом. */
export async function toggleScriptPair() {
  state.scriptShown = !state.scriptShown;
  await setSetting('scriptShown', state.scriptShown);
  renderSettings();
}
...
export function renderSettings() {
  ...
  document.getElementById('settings-theme').checked = dark;
  ...
  limit.value = String(state.sessionLimit);
  syncRangeFill(limit);

  ensureNiqqudCard();
  document.getElementById('niqqud-toggle').checked = !state.niqqudHidden;
  document.getElementById('script-toggle').checked = state.scriptShown;
...
/** Доля скачанного на кнопке. Полоса растёт через scaleX — width пересчитывал бы раскладку. */
function setOfflineFill(share) {
  document.getElementById('offline-btn').style.setProperty('--fill', String(Math.min(1, share)));
}
...
export async function refreshOfflineStatus() {
  ...
  try {
    ...
    // Кнопка сама показывает долю: полоса под подписью растёт вместе со скачанным
    setOfflineFill(total ? ready / total : 0);
    button.classList.toggle('is-done', left === 0);
    if (left === 0) {
      fill('offline-label', iconLabel('check', 'Всё скачано'));
      status.textContent = `Все ${total} записей лежат в памяти — интернет для озвучки не нужен.`;
    } else if (ready === 0) {
      fill('offline-label', 'Скачать всю озвучку');
      ...
      fill('offline-label', `Докачать ${left}`);
...
export async function cacheAllAudio() {
  const button = document.getElementById('offline-btn');
  const status = document.getElementById('offline-status');
  // «Занята» — не «выключена»: кнопку нельзя гасить, иначе кажется, что она сломалась
  ...
  try {
    ...
    const worker = async () => {
      while (urls.length) {
        ...
        if (done % 10 === 0) {
          setOfflineFill(done / total);
          fill('offline-label', `Скачано ${done} из ${total}`);
        }
      }
    };
...
export async function importFromFile(file) {
  let applying = false;          // дошли ли до записи в базу — от этого зависит текст ошибки
  try {
    ...
    const agreed = await askConfirm({
      ...
      hint: 'Слова, которые уже есть, останутся как есть. Прогресс сливается: по каждому слову '
        + 'остаётся более свежая запись, у дня статистики берётся большее из двух чисел, '
        + 'открытый уровень, пройденные уроки и день программы назад не откатятся.',
    ...
    /* Разбор файла и его применение падают по-разному: во втором случае часть слов
       уже записана, и человеку важно знать, что база не осталась прежней. */
    toast(applying
      ? `Загрузка оборвалась: ${error.message}. Часть слов могла записаться — попробуй тот же файл ещё раз.`
      : `Не вышло прочитать файл: ${error.message}`, true);
  }
}
...
export async function resetProgress() {
  const agreed = await askConfirm({
    ...
    text: 'Удалятся интервалы повторений, статистика по дням и результаты экзаменов, '
      + 'уровни закроются до первого. Слова, отметки букв, трудные слова, уроки грамматики '
      + 'и программа занятий останутся на месте.',
