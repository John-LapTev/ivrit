...
import { openDialog } from './dialogs-screen.js';
...
export function runTeacherStep(entry, step) {
  ...
  if (step === 'dialog') {
    if (!entry.dialog) { toast('У этого дня нет своей сценки.', true); return; }
    state.teacherReturn = { day: entry.day, step: 'dialog' };
    openDialog(entry.dialog, 'teacher-day');
    return;
  }
