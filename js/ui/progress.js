...
/* Свой номер у каждой отрисовки графика: id градиента обязан быть уникальным на странице. */
let chartPaintCounter = 0;
...
function renderChart() {
  ...
  const svg = svgEl('svg', {
    viewBox: `0 0 ${width} ${height + 16}`, class: 'chart', role: 'img',
    'aria-label': `График выученных слов за ${CHART_DAYS} дней`,
  });

  /* Столбики красим акцент-градиентом дизайн-системы — он живёт в defs самого SVG.
     Свой id на каждую отрисовку: два графика на экране перебивали бы заливку друг другу
     (та же грабля, что у рисунков ранга — см. rank.js). */
  chartPaintCounter += 1;
  const gradientId = `chart-gradient-${chartPaintCounter}`;
  ...
  days.forEach((day, index) => {
    ...
    const rect = svgEl('rect', {
      x: index * (barWidth + gap), y: height - barHeight,
      width: barWidth, height: barHeight, rx: 1, fill: `url(#${gradientId})`,
    });
  ...
  svg.append(svgEl('line', {
    class: 'chart-axis', x1: 0, y1: height, x2: width, y2: height,
  }));

  const labelLeft = svgEl('text', { x: 0, y: height + 12 });
  labelLeft.textContent = `${CHART_DAYS} дней назад`;
  const labelRight = svgEl('text', { x: width, y: height + 12, 'text-anchor': 'end' });
