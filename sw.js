 ...
 */
const CACHE = 'ivrit-v13';          // имя проверяет tools/publish.sh — не переименовывать
...
/** Страница ли это: только ей осмысленно отдать сохранённый index.html. */
const isPage = (url) => url.pathname.endsWith('/') || url.pathname.endsWith('.html');

/** Честный ответ, когда офлайн и в кеше пусто: пустой 504, а не чужое содержимое. */
const offlineAnswer = (url) => new Response('', {
  status: 504,
  statusText: 'Офлайн: файла нет в сохранённой копии',
});
  ...
  if (isCode) {
    event.respondWith(
      fetch(event.request, { cache: 'no-cache' }).then((response) => {
        ...
        // Офлайн — отдаём, что есть. Запасная страница только вместо страницы:
        // подсунуть HTML вместо .js сломает запуск молча.
        (hit) => hit || (isPage(url) ? caches.match('./index.html') : undefined)
      ).then((response) => response || offlineAnswer(url)))
    );
    return;
  }

  /* Шрифты и иконки — редко меняются, их быстрее брать из кеша. Заменять их страницей
     нельзя: браузер отвергнет HTML как шрифт и возьмёт системный, а он рвёт огласовки. */
  event.respondWith(
    caches.match(event.request)
      .then((hit) => hit || fetch(event.request).catch(() => offlineAnswer(url)))
  );
});
